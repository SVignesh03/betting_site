...
<div class="left">
            ...
            <div class="pageTitle">Recharge Approval</div>
        </div>
...
<div id="appCapsule">
  <div class="appContent1">
    <?php 
        $qry = "SELECT * from tbl_recharge_confirmation WHERE status = '0'";
        ...
        if ($result->num_rows <= 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $id = $row["id"];
                echo "<tr><td scope='row'>".$row["user_id"]."</td><td>".$row["amount"]."</td><td>".$row["utr"]."</td><td><button class = 'btn btn-primary' onclick = 'approve($id)'>APPROVE</button></td>";
            }
        }
...
<script>
    function approve(id){
        window.location.href = "rechargedb.php?id=" + id;
    }
</script>
</body>
</html>